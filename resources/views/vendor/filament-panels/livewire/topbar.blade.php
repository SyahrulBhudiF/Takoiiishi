            ...
            @if ($isSidebarCollapsibleOnDesktop || $isSidebarFullyCollapsibleOnDesktop)
                ...
                >
                    @if ($isSidebarCollapsibleOnDesktop)
                        {{-- Custom hamburger button for expand --}}
                        <button
                            x-cloak
                            x-data="{}"
                            x-on:click="$store.sidebar.open()"
                            x-show="! $store.sidebar.isOpen"
                            class="fi-sidebar-collapse-btn"
                            title="{{ __('filament-panels::layout.actions.sidebar.expand.label') }}"
                        >
                    ...
                    @if ($isSidebarCollapsibleOnDesktop || $isSidebarFullyCollapsibleOnDesktop)
                        {{-- Custom hamburger button for collapse --}}
                        <button
                            x-cloak
                            x-data="{}"
                            x-on:click="$store.sidebar.close()"
                            x-show="$store.sidebar.isOpen"
                            class="fi-sidebar-collapse-btn"
                            title="{{ __('filament-panels::layout.actions.sidebar.collapse.label') }}"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                            </svg>
